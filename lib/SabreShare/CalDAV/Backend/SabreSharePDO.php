...
//			}
    ...
    public function getCalendarsForUser($principalUri) {
        ...
        $this->logInfo("Getting calendars for: " . $principalUri);
        ...
        while($row = $stmt->fetch(\PDO::FETCH_ASSOC)) {
            ...
            $this->logInfo("Found a normal calendar for ".$row['id']." and ".$row['uri']);
        ...
        while($shareRow = $shareStmt->fetch(\PDO::FETCH_ASSOC)) {
            ...
            $this->logInfo("Found a share for ".$principal['id'].", calendar id is: ".$shareRow['calendarId']);
            ...
            while($calendarShareRow = $calStmt->fetch(\PDO::FETCH_ASSOC)) {
                ...
                $this->logInfo("Found a shared calendar with id ".$calendarShareRow['id'].", uri is: ".$calendarShareRow['uri']);
...
// 					}
        ...
        $this->logInfo("Found a total of ".count($calendars)." calendars");
    ...
    private function setupLog() {
        ...
        $this->log = new Logger('SabreSharePDO');
        $this->log->pushHandler(new StreamHandler('/var/log/baikal/logfile.txt', Logger::INFO));
    }
